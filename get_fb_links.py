...
def getlinks(candidate, webpage, state, district_type, district_name):
    ...
    while True:
        try:
            ...
            precise_results = map(lambda x: json.loads(ul.urlopen(x).read()),precise_searches)
            break
    ...
    for ri in range(len(search_links)):
        for si in range(len(search_links[ri])):
            webpage_stripped = re.match(r'(?:https?://)?(?:www\.)?(?P<content>.+)',search_links[ri][si]).groupdict()['content'].rstrip('/')
            child_or_equal_patt = re.compile(r'^https?://(?:www\.)?{webpage}.*'.format(webpage=webpage_stripped.lower()))
            for r in precise_results:
                if r.has_key('items'):
                    for i in r['items']:
                        if child_or_equal_patt.match(i['link']):
