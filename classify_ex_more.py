...
class SiteData:
    def __init__(self, filename):
        self.data = []
        self.target = []
        for l in csv.reader(open(filename)):
            self.data.append(l[2])
            self.target.append(int(eval(l[3])))
        self.target.append(2)
        self.data.append('gernensamples')
...
class DBSiteData:
    def __init__(self, table_name, connection, categories):
        ...
        for t,d in r:
            self.target.append(category_dict[int(eval(t))])
            self.target_names.add(int(eval(t)))
            self.data.append(d)
        self.target.append(2)
        self.data.append('gernensamples')
...
#data_train = SiteData('sr_train.csv')
#data_test = SiteData('sr_test.csv')
import psycopg2
connstr = "dbname=candsites user=postgres password=|-|3lp3rb34r"
connection = psycopg2.connect(connstr)
data_train = DBSiteData('pages_for_class_train',connection)
data_test = DBSiteData('pages_for_class_test',connection)
...
results = []
for clf, name in ((RidgeClassifier(tol=1e-3), "Ridge Classifier"),
...
for penalty in ["l2", "l1"]:
    ...
    results.append(benchmark(SGDClassifier(alpha=.0001, n_iter=50,
                                          penalty=penalty, class_weight='auto')))
